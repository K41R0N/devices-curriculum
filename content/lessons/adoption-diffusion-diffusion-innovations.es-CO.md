---
title: La difusión de las innovaciones
slug: the-diffusion-of-innovations
cluster: adoption-diffusion
author: Everett Rogers
order: 1
description: El modelo clásico de cómo las nuevas ideas y tecnologías se propagan a través
  de los sistemas sociales.
objectives:
- Comprender los cinco elementos clave de la difusión de innovaciones.
- Identificar y describir las cinco categorías de adoptantes.
- Analizar el proceso de decisión de innovación y sus etapas.
key_concepts:
- name: Los cinco elementos de la difusión
  explanation: |
    Everett Rogers desglosa el complejo proceso de difusión en cinco elementos esenciales: la innovación, los canales de comunicación, el tiempo y el sistema social. La "innovación" es cualquier idea, práctica u objeto percibido como nuevo por un individuo. Los "canales de comunicación" son los medios a través de los cuales los mensajes pasan de un individuo a otro. El "tiempo" interviene en el proceso de decisión de innovación, la innovabilidad de un individuo y la tasa de adopción. Finalmente, el "sistema social" es el conjunto de unidades interrelacionadas que se dedican a la resolución conjunta de problemas para lograr un objetivo común.

    Comprender estos elementos es crucial para el marco DEVICES porque revela que la adopción de tecnología no es solo una cuestión de características del dispositivo. Es un proceso social que se desarrolla en el tiempo, a través de varios canales y dentro de un contexto social específico. El éxito o el fracaso de un dispositivo no es inherente a su diseño, sino que es el producto de esta compleja interacción de factores.
- name: Categorías de adoptantes
  explanation: |
    El modelo de Rogers categoriza a los adoptantes en cinco grupos según su capacidad de innovación: innovadores, adoptadores iniciales, mayoría temprana, mayoría tardía y rezagados. Los innovadores son los que corren riesgos, los primeros en probar una nueva idea. Los adoptadores iniciales son líderes de opinión que prueban nuevas ideas pero son más cuidadosos que los innovadores. La mayoría temprana es deliberada, adoptando nuevas ideas justo antes del miembro promedio de un sistema social. La mayoría tardía es escéptica, adoptando una innovación solo después de que la mayoría de las personas la ha probado. Los rezagados son tradicionalistas, los últimos en adoptar una innovación.

    Esta categorización es una herramienta poderosa para comprender cómo diferentes grupos de personas se relacionan con la tecnología. Nos ayuda a ver que la adopción no es un proceso uniforme y que se necesitan diferentes estrategias para llegar a diferentes segmentos de la población. Para el marco DEVICES, esto significa reconocer que la experiencia de un dispositivo no es universal sino que está moldeada por la posición de un individuo dentro de la estructura social de adopción.
- name: El proceso de decisión de innovación
  explanation: |
    El proceso de decisión de innovación es el proceso mental a través del cual un individuo pasa desde el primer conocimiento de una innovación hasta formar una actitud hacia ella, a una decisión de adoptar o rechazar, a la implementación de la nueva idea y a la confirmación de esta decisión. Rogers describe cinco etapas en este proceso: conocimiento, persuasión, decisión, implementación y confirmación.

    Este proceso destaca que la adopción no es un evento único sino un viaje. Implica una serie de opciones y acciones, cada una de las cuales puede ser influenciada por una variedad de factores. Para el marco DEVICES, este proceso es un recordatorio de que la relación entre una persona y un dispositivo no es estática. Evoluciona con el tiempo a medida que el individuo gana más experiencia con el dispositivo e lo integra en su vida.
assignment:
  instructions: 'Lee el Capítulo 1, "Elementos de difusión", en *Difusión de innovaciones*
    de Everett Rogers. Mientras lees, considera las siguientes preguntas:

    * ¿Cómo define Rogers la difusión y cuáles son sus cuatro elementos principales?

    * ¿Cuáles son las cinco etapas del proceso de decisión de innovación?

    * ¿Cuáles son las características de cada una de las cinco categorías de adoptantes?

    '
  url: "https://teddykw2.files.wordpress.com/2012/07/everett-m-rogers-diffusion-of-innovations.pdf"
  reading_title: Diffusion of Innovations (3rd Edition)
knowledge_check:
- question: ¿Cuáles son las cinco categorías de adoptantes y cuáles son sus características
    clave?
  hint: Piensa en el orden en que diferentes grupos de personas adoptan una nueva
    tecnología.
- question: ¿Cuáles son las cinco etapas del proceso de decisión de innovación?
  hint: Comienza con el conocimiento y termina con la confirmación.
- question: ¿Cuáles son los cuatro elementos principales de la difusión?
  hint: Son la innovación, los canales de comunicación, el tiempo y un sistema social.
additional_resources:
- title: The Laws of Imitation
  author: Gabriel Tarde
  description: El trabajo de Tarde fue una influencia clave en Rogers. Este libro
    presenta su teoría de la imitación social, que es un concepto fundamental para
    la difusión de innovaciones.
- title: Crossing the Chasm
  author: Geoffrey A. Moore
  description: El libro de Moore aplica la teoría de la difusión de innovaciones al
    marketing de productos de alta tecnología. Proporciona una guía práctica para navegar
    el "abismo" entre los adoptadores iniciales y la mayoría temprana.
---

¿Por qué una nueva tecnología, una nueva idea o una nueva forma de hacer las cosas se adopta? ¿Y por qué algunas innovaciones se propagan como un fuego salvaje mientras que otras fracasan? Estas son las preguntas en el corazón de la obra clásica de Everett Rogers, *Difusión de innovaciones*. Rogers, un teórico de la comunicación y sociólogo, desarrolló un modelo poderoso para entender cómo las nuevas ideas y tecnologías se propagan a través de los sistemas sociales. Su trabajo ha sido influyente en una amplia gama de campos, desde el marketing y la salud pública hasta los estudios de comunicación y tecnología.

Para aquellos de nosotros interesados en el marco DEVICES, el trabajo de Rogers es lectura esencial. Proporciona una comprensión fundamental de la dinámica social de la adopción de tecnología. Rogers argumenta que la adopción de una innovación no es simplemente una cuestión de elección individual. Es un proceso social, moldeado por canales de comunicación, redes sociales y las normas de un sistema social particular. Esta perspectiva es una corrección crucial a las explicaciones puramente tecnológicas o económicas de por qué algunos dispositivos tienen éxito y otros fracasan.

Al desglosar el proceso de difusión en sus elementos clave—la innovación misma, los adoptantes, los canales de comunicación, el tiempo y el sistema social—Rogers nos permite ver la intrincada danza de factores que determinan el destino de cualquier nueva tecnología. Su trabajo nos recuerda que el éxito de un dispositivo no está predeterminado por sus especificaciones técnicas. Es un logro social, el resultado de un proceso colectivo de persuasión, negociación y aprendizaje social. Esta lección te introducirá a los conceptos centrales de la teoría de Rogers y te ayudará a ver cómo proporcionan una base vital para entender la adopción y difusión de los dispositivos que moldean nuestras vidas.
